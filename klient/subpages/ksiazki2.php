						...
						try{
							...
							if($poloczenie->connect_errno != 0){
								...
								$query = $poloczenie->query(
									...
									FROM 
									(SELECT * FROM książki) książki 
									INNER JOIN wydawnictwa ON wydawnictwa.ID_wydawnictwa=książki.ID_wydawnictwa 
									
									INNER JOIN kategorie ON kategorie.ID_kategorii=książki.ID_kategorii WHERE książki.Tytuł LIKE '%$wynik%'"
								);
								...
								if($query->num_rows == 0)
								{
									echo "<h1>Brak wyników wyszukiwania</h2>";
