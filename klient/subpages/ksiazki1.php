				...
				<div class="collapse navbar-collapse" id="list">
					<ul class="navbar-nav mr-5">
						...
						<li class="nav-item dropdown mr-4">
							<a class="nav-link dropdown-toggle active" data-toggle="dropdown" role="button"><i class="fas fa-book mr-2"></i>Książki</a>
			...
			<div class="container-fluid">
				...
				<form action="ksiazki1.php" method="post" autocomplete="off">
					...
					<div class="h2 container-group border-top"> Rok wydania </div>
						<div class="form-group">
						<input class="form-control" name="rokwydania" placeholder="Wprowadź..">
						</div>
					<div class="h2 container-group border-top"> Cena </div>
						<div class="form-group">
							<input class="form-control mb-2" name="mincena" id="odcena" placeholder="Od-zł">
							<input class="form-control" name="maxcena" id="docena" placeholder="Do-zł">
						</div>
						<button id="submit" class="btn btn-dark btn-block mt-4 mb-2" type="submit">Filtruj</button>
						<button id="submit" class="btn btn-danger btn-block mb-3" type="submit">Wyczyść</button>
				   </form>
                </div>
				<div class="d-block ml-auto col-8 col-sm-8 col-md-9 col-lg-10 container-group border-left">
					
					<?php
					if(isset($_POST['autor']) || isset($_POST['wydawnictwo']) || isset($_POST['rokwydania']) || isset($_POST['mincena']) || isset($_POST['maxcena'])){
						require_once "connect.php";
						mysqli_report(MYSQLI_REPORT_STRICT);
						try{
							$poloczenie = new mysqli($host, $db_user, $db_pass, $db_name);
							if($poloczenie->connect_errno != 0){
								...
								if(isset($_POST['maxcena'])){
									$cenamax = $_POST['maxcena'];
								}

								foreach($autor as $a){
									echo '<span class="bg-success text-white" style="border-radius:6px;">Autor: '.$a.'</span> ';
								}

								foreach($wydawnictwo as $w){
									echo '<span class="bg-success text-white" style="border-radius:6px;">Wydawnictwo: '.$w.'</span> ';
								}
								if($rokwydania != "") echo '<span class="bg-success text-white" style="border-radius:6px;">Rok wydania: '.$rokwydania.'</span> ';
								if($cenamin != "") echo '<span class="bg-success text-white" style="border-radius:6px;">Cena od: '.$cenamin.'</span> ';
								if($cenamax != "") echo '<span class="bg-success text-white" style="border-radius:6px;">Cena do: '.$cenamax.'</span> ';
								echo '<a href="ksiazki.php?page=1" class="btn btn-dark"><i class="fas fa-times"></i></a>';
								$sql = "SELECT DISTINCT książki.Tytuł, książki.Rok_wydania, książki.Cena, książki.Liczba_sztuk, wydawnictwa.Nazwa, autorzy.Imie, autorzy.Nazwisko, kategorie.Nazwa, książki.ID_ksiązki 
								FROM książki 
								INNER JOIN wydawnictwa ON wydawnictwa.ID_wydawnictwa=książki.ID_wydawnictwa 
								INNER JOIN książki_autorzy ON książki_autorzy.ID_książki=książki.ID_ksiązki 
								INNER JOIN autorzy ON autorzy.ID_autora=książki_autorzy.ID_autora 
								INNER JOIN kategorie ON kategorie.ID_kategorii=książki.ID_kategorii WHERE ";

								if($autor != null){
									if(count($autor) == 1){
										$z = $autor[0];
										$sql .= "autorzy.Nazwisko='$z'";
									}
									else{
										$z1 = $autor[0];
										$sql .= "autorzy.Nazwisko='$z1'";
										for($i = 1; $i<count($autor); $i++){
											$z2 = $autor[$i];
											$sql .= " OR ";
											$sql .= "autorzy.Nazwisko='$z2'";
										}
									}
								}
								...
								if($wydawnictwo != null){
									...
									if(count($wydawnictwo) == 1){
										$z = $wydawnictwo[0];
										$sql .= "wydawnictwa.Nazwa='$z'";
									}
									else{
										$z1 = $wydawnictwo[0];
										$sql .= "wydawnictwa.Nazwa='$z1'";
										for($i = 1; $i<count($wydawnictwo); $i++){
											$z2 = $wydawnictwo[$i];
											$sql .= " OR ";
											$sql .= "wydawnictwa.Nazwa='$z2'";
										}
									}
								}

								if($rokwydania != ""){
									if($autor != null && $wydawnictwo == null){
										$sql .= " OR ";
									}
									if($wydawnictwo != null && $autor == null){
										$sql .= " OR ";
									}
									if($wydawnictwo != null && $autor != null){
										$sql .= " OR ";
									}
									$sql .= "książki.Rok_wydania='$rokwydania'";
								}

								if($cenamin != "" && $cenamax == ""){
									if($autor != null && $wydawnictwo == null && $rokwydania == ""){
										$sql .= " OR ";
									}
									if($wydawnictwo != null && $autor == null && $rokwydania == ""){
										$sql .= " OR ";
									}

									if($rokwydania != "" && $wydawnictwo == null && $autor == null){
										$sql .= " OR ";
									}
								...
								if($cenamax != "" && $cenamin == ""){
									...
									if($rokwydania != "" && $autor == null && $wydawnictwo == null){
										$sql .= " OR ";
									}
									if($autor != null && $wydawnictwo != null && $rokwydania != ""){
										$sql .= " OR ";
									}
									$sql .= "książki.Cena<'$cenamax'";
								}
								...
								if($cenamin != "" && $cenamax != ""){
									...
									$sql .= "książki.Cena>='$cenamin' && książki.Cena<='$cenamax'";
								}

								$sql .= "GROUP BY książki.ID_ksiązki";

								$query = $poloczenie->query($sql);
								...
								if($ile_rekordow != 0){
									...
									while($row = mysqli_fetch_row($query)){
										$zap1 = $poloczenie->query("SELECT autorzy.Imie, autorzy.Nazwisko FROM książki_autorzy INNER JOIN autorzy ON książki_autorzy.ID_autora = autorzy.ID_autora WHERE książki_autorzy.ID_książki = ".$row[8]."");
											$autorzy = "";
											...
											echo '<div class="thumbnail col-10 mt-3 mb-3 col-md-4 col-sm-4 col-lg-2 border-bottom d-flex flex-column" style="text-align:center;"><a href="book.php?tytul='.$row[0].'&rokwydania='.$row[1].'&cena='.$row[2].'&liczbasztuk='.$row[3].'&wydawnictwo='.$row[4].'&autor='.$autorzy.'&kategoria='.$row[7].'&id='.$row[8].'" style="text-decoration:none; color:black;">'; 
												...
												echo '</div></a>';
												if($row[3] >0 )
													echo '<a class="mt-auto" href="koszyk.php?tytul='.$row[0].'&ilosc=1&cena='.$row[2].'" style="text-decoration:none;"><button class="btn btn-secondary btn-block mb-2 mt-auto" type="button">Dodaj do koszyka</button></a>';
												else
