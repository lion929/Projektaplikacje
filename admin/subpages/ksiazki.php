...
<html>
    ...
    <body>
        ...
        <main>
            ...
            <div class="container text-center mb-3">
                ...
                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="radiobtn" id="radio1" value=2 checked>
                    <label class="form-check-label text-warning font-weight-bold" for="radio1">Tytuł</label>
                </div>  

                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="radiobtn" id="radio2" value=0>
                    <label class="form-check-label text-warning font-weight-bold" for="radio2">Kategoria</label>
                </div>

                <div class="form-check form-check-inline">
                    <input class="form-check-input" type="radio" name="radiobtn" id="radio3" value=1>
                    <label class="form-check-label text-warning font-weight-bold" for="radio3">Wydawnictwo</label>
                </div>          
            </div>
            ...
            <nav class="navbar navbar-light navbar-expand-lg">
				...
				<div class="collapse navbar-collapse" id="nav">
					...
					<form class="form-inline ml-auto mr-auto">
                        ...
                        <a href="dodawanie/ksiazki_dodawanie.php" class="btn text-light bg-success mr-5" role="button"><i class="fas fa-plus"></i> Dodaj rekord</a>
                          ...
                          <tbody>
                            <?php
                                ...
                                try {
                                    ...
                                    {
                                        $query = "SELECT kategorie.Nazwa as kat, wydawnictwa.Nazwa as wyd, książki.ID_ksiązki, książki.Tytuł, książki.Rok_wydania, książki.Cena, książki.Liczba_sztuk 
                                        FROM książki INNER JOIN kategorie ON książki.ID_kategorii = kategorie.ID_kategorii 
                                        INNER JOIN wydawnictwa ON książki.ID_wydawnictwa = wydawnictwa.ID_wydawnictwa ORDER BY książki.ID_ksiązki DESC"; 
                                        ...
                                        else
                                        {
                                            while ($row = $result->fetch_assoc())
                                            {
                                                ...
                                                echo "<tr id='row$id'><td>".$row['kat']."</td><td>".$row['wyd']."</td><td>".$row['Tytuł']."</td><td>".
                                                $row['Rok_wydania']."</td><td>".$row['Cena']."</td><td>".$row['Liczba_sztuk']."</td>";

                                                echo "<td><a href='ksiazki_autorzy.php?id=$id' class='btn btn-secondary'><i class='fas fa-user-friends mr-2'></i>Autorzy<i class=''></i></a></td>";
                                                echo "<td><a href='edycja/ksiazki_edycja.php?id=$id' class='btn btn-primary' title='Edytuj'><i class='fas fa-edit'></i></a></td>";
                                                echo "<td><button class='btn btn-danger' title='Usuń' onclick='delete_row($id)'><i class='fas fa-trash-alt'></i></a></td></tr>";
                                            }
        ...
        <script>
            ...
            {
                $.ajax({
                    url: "usuwanie/ksiazki_usuwanie.php",
                    type: "get",
                    data: "id="+id,
                    ...
                    { 
                       if(data==1)
                       {
                        ...
                        {
                            alert("Nie można usunąć książki, ponieważ istnieją zamówienia zawierające tę książkę");
                        }
