<?php
    ...
    try {
		...
		{
			$query = "SELECT * FROM kategorie WHERE ID_kategorii = '$id'";
    ...
    if(isset($_POST['nazwa'])){
		...
		try {
			...
			if($conn1->connect_errno != 0){
				...
				$check = $conn1->query("SELECT * FROM kategorie WHERE Nazwa='$nazwa1'");
				...
				if($ok == true){
					...
					if($conn1->query("UPDATE kategorie SET Nazwa = '$nazwa1' WHERE ID_kategorii=$id")){
