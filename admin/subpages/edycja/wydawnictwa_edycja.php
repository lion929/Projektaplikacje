<?php
    ...
    try {
		...
		{
			$query = "SELECT * FROM wydawnictwa WHERE ID_wydawnictwa = '$id'";
    ...
    if(isset($_POST['nazwa'])){
		...
		try {
			...
			if($conn1->connect_errno != 0){
				...
				$check = $conn1->query("SELECT * FROM wydawnictwa WHERE Nazwa='$nazwa1'");
				...
				if($count > 0) {
					$ok = false;
					$_SESSION['err_name'] = "<script> document.getElementById('name').innerHTML = 'Istnieje już wydawnictwo o takiej nazwie'; </script>";
				}

				if($ok == true){
					
					if($conn1->query("UPDATE wydawnictwa SET Nazwa = '$nazwa1' WHERE ID_wydawnictwa=$id")){
        ...
        <div id="content">
            ...
            <div class="container col-md-4">
                ...
                <form action="" method="post" autocomplete="off">
					...
					<label for="nazwaa">Nazwa</label>
					<input type="text" class="form-control bg-dark text-light d-block" name="nazwa" id="nazwaa" placeholder="Nazwa" value="<?php echo $nazwa ?>" required disabled>
					...
					<div class="d-flex justify-content-center">
						<button id="submit" class="btn btn-primary btn-block mt-5 mb-4 w-75" type="submit" disabled><i class="fas fa-check mr-2"></i>Zapisz zmiany</button>
					</div>
					<div class="d-flex justify-content-center">
						<a href="../wydawnictwa.php" class="btn btn-outline-warning btn-block w-75"><i class="fas fa-arrow-left mr-2"></i>Powrót</a>
					</div>
                </form>
